...
export const removeRestaurant = async(restaurantId) => {
    ...
    try {
        const deletionResult = await reviewCollection.deleteMany({ dishId: { $in: dishIds } });
        if (deletionResult.acknowledged) console.log(`Successfully deleted ${deletionResult.deletedCount} reviews.`);
        else console.log("Failed to delete reviews.");
    } catch (error) {
